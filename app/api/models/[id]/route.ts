...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (usePostgres()) {
      ...
      const value = { displayName: typeof body.displayName === "string" ? body.displayName.trim() : row.displayName, visible: typeof body.visible === "boolean" ? body.visible : row.visible, enabled: typeof body.enabled === "boolean" ? body.enabled : row.enabled, updatedAt: Date.now() };
